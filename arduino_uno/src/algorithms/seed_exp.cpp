#include <Arduino.h>
#include <string.h>

#include "exp_common.h"

namespace {

const uint32_t s_kc[16] = {0x9e3779b9, 0x3c6ef373, 0x78dde6e6, 0xf1bbcdcc, 0xe3779b99, 0xc6ef3733, 0x8dde6e67, 0x1bbcdccf,
	0x3779b99e, 0x6ef3733c, 0xdde6e678, 0xbbcdccf1, 0x779b99e3, 0xef3733c6, 0xde6e678d, 0xbcdccf1b};

const uint8_t s_s0[256] = {};

  const uint8_t s_s1[256] = {};

  #define SS0(x) ((s_s0[x]*0x01010101UL) & 0x3FCFF3FC)
  #define SS1(x) ((s_s1[x]*0x01010101UL) & 0xFC3FCFF3)
  #define SS2(x) ((s_s0[x]*0x01010101UL) & 0xF3FC3FCF)
  #define SS3(x) ((s_s1[x]*0x01010101UL) & 0xCFF3FC3F)
  #define GETBYTE(x, y) static_cast<uint8_t>(((x) >> (8U * (y))) & 0xFFU)
  #define G(x) (SS0(GETBYTE(x, 0)) ^ SS1(GETBYTE(x, 1)) ^ SS2(GETBYTE(x, 2)) ^ SS3(GETBYTE(x, 3)))

  static inline uint32_t load32be(const uint8_t* p) {
    return (static_cast<uint32_t>(p[0]) << 24) |
           (static_cast<uint32_t>(p[1]) << 16) |
           (static_cast<uint32_t>(p[2]) << 8) |
           static_cast<uint32_t>(p[3]);
  }

  static inline void store32be(uint8_t* p, uint32_t v) {
    p[0] = static_cast<uint8_t>(v >> 24);
    p[1] = static_cast<uint8_t>(v >> 16);
    p[2] = static_cast<uint8_t>(v >> 8);
    p[3] = static_cast<uint8_t>(v);
  }

  void seed_set_key(const uint8_t user_key[16], uint32_t rkey[32], uint32_t drkey[32]) {
    uint32_t k1 = load32be(user_key + 0);
    uint32_t k2 = load32be(user_key + 4);
    uint32_t k3 = load32be(user_key + 8);
    uint32_t k4 = load32be(user_key + 12);

    for (uint8_t i = 0; i < 16; ++i) {
      rkey[2 * i + 0] = G(k1 + k3 - s_kc[i]);
      rkey[2 * i + 1] = G(k2 - k4 + s_kc[i]);

      if (i & 1U) {
        const uint32_t t = k3;
        k3 = (k3 << 8) | (k4 >> 24);
        k4 = (k4 << 8) | (t >> 24);
      } else {
        const uint32_t t = k1;
        k1 = (k1 >> 8) | (k2 << 24);
        k2 = (k2 >> 8) | (t << 24);
      }

      drkey[2 * (15 - i) + 0] = rkey[2 * i + 0];
      drkey[2 * (15 - i) + 1] = rkey[2 * i + 1];
    }
  }

  void seed_crypt_block(const uint32_t rkey[32], const uint8_t in[16], uint8_t out[16]) {
    uint32_t p0 = load32be(in + 0);
    uint32_t p1 = load32be(in + 4);
    uint32_t p2 = load32be(in + 8);
    uint32_t p3 = load32be(in + 12);

    for (uint8_t i = 0; i < 16; i += 2) {
      uint32_t t0 = p2 ^ rkey[2 * i + 0];
      uint32_t t1 = p3 ^ rkey[2 * i + 1] ^ t0;
      t1 = G(t1);
      t0 += t1;
      t0 = G(t0);
      t1 += t0;
      t1 = G(t1);
      p0 ^= t0 + t1;
      p1 ^= t1;

      t0 = p0 ^ rkey[2 * i + 2];
      t1 = p1 ^ rkey[2 * i + 3] ^ t0;
      t1 = G(t1);
      t0 += t1;
      t0 = G(t0);
      t1 += t0;
      t1 = G(t1);
      p2 ^= t0 + t1;
      p3 ^= t1;
    }

    store32be(out + 0, p2);
    store32be(out + 4, p3);
    store32be(out + 8, p0);
    store32be(out + 12, p1);
  }

  bool seed_self_test() {
    const uint8_t tv_pt1[16] = {
      0x00,0x01,0x02,0x03,0x04,0x05,0x06,0x07,0x08,0x09,0x0A,0x0B,0x0C,0x0D,0x0E,0x0F
    };
    const uint8_t tv_ct1[16] = {
      0x5E,0xBA,0xC6,0xE0,0x05,0x4E,0x16,0x68,0x19,0xAF,0xF1,0xCC,0x6D,0x34,0x6C,0xDB
    };
    const uint8_t tv_key1[16] = {0};

    const uint8_t tv_pt2[16] = {0};
    const uint8_t tv_ct2[16] = {
      0xC1,0x1F,0x22,0xF2,0x01,0x40,0x50,0x50,0x84,0x48,0x35,0x97,0xE4,0x37,0x0F,0x43
    };
    const uint8_t tv_key2[16] = {
      0x00,0x01,0x02,0x03,0x04,0x05,0x06,0x07,0x08,0x09,0x0A,0x0B,0x0C,0x0D,0x0E,0x0F
    };

    uint32_t rk[32], drk[32];
    uint8_t buf[16];

    seed_set_key(tv_key1, rk, drk);
    seed_crypt_block(rk, tv_pt1, buf);
    if (memcmp(buf, tv_ct1, sizeof(buf)) != 0) {
      return false;
    }
    seed_crypt_block(drk, buf, buf);
    if (memcmp(buf, tv_pt1, sizeof(buf)) != 0) {
      return false;
    }

    seed_set_key(tv_key2, rk, drk);
    seed_crypt_block(rk, tv_pt2, buf);
    if (memcmp(buf, tv_ct2, sizeof(buf)) != 0) {
      return false;
    }
    seed_crypt_block(drk, buf, buf);
    return memcmp(buf, tv_pt2, sizeof(buf)) == 0;
  }

#undef G
#undef GETBYTE
#undef SS3
#undef SS2
#undef SS1
#undef SS0
}


void run_seed_experiments(void) {
  const uint8_t key[16] = {0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07,
                           0x08, 0x09, 0x0A, 0x0B, 0x0C, 0x0D, 0x0E, 0x0F};
  const uint8_t in_block[16] = {0x10, 0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x17,
                                0x18, 0x19, 0x1A, 0x1B, 0x1C, 0x1D, 0x1E, 0x1F};

  if (!seed_self_test()) {
    print_unsupported_algo(F("seed"), 16, 16, key[0], in_block[0],
                           F("self-test failed"));
    return;
  }

  uint32_t rkey[32];
  uint32_t drkey[32];
  uint8_t out_block[16] = {0};
  seed_set_key(key, rkey, drkey);

  unsigned long time_trials = 0UL;
  for (int i = 0; i < 200; ++i) {
    const unsigned long start = micros();
    seed_crypt_block(rkey, in_block, out_block);
    seed_crypt_block(drkey, out_block, out_block);
    time_trials += (micros() - start);
  }

  print_experiment_row(F("seed_ref"), 16, 16, time_trials / 200UL, key[0], out_block[0],
                       F("SEED standalone"));
}
