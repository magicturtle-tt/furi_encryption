...
void run_lea_experiments(void) {
    const unsigned key_bytes = CryptoPP::LEA::DEFAULT_KEYLENGTH;
    const unsigned block_bytes = CryptoPP::LEA::BLOCKSIZE;
    ...
    CryptoPP::LEA::Encryption enc;
    CryptoPP::LEA::Decryption dec;
    enc.SetKey(key, key_bytes);
    dec.SetKey(key, key_bytes);
    enc.ProcessBlock(in_block, out_block);

    absolute_time_t start = get_absolute_time();
    dec.ProcessBlock(out_block); // this will decrypt out_block and store it in out_block. we only care to measure the decrption part
    ...
    printf("lea_cryptopp,%u,%u,%lld,%02x,%02x,%s\n",
           key_bytes,
           block_bytes,
